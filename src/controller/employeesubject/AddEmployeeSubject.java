...
public class AddEmployeeSubject extends HttpServlet {
	/**
	 * 
	 */
	...
	protected void doprocess(HttpServletRequest request, HttpServletResponse response) throws IOException{
		...
		try {
			...
			empSub.setEmp_id(1);
			empSub.setSub_id(17);
			empSub.setEmpsub_rate(3);
			empSub.setEmpsub_desc("test emp");
			boolean state = new EmployeeSubjectDAO().addEmployeeSubject(empSub);
			if(state){
